...
export function enhanceItem({ props, options, caso }: { props: UpgradeInfo, options: OptionType, caso: number }) {
    while (props.enhance.now! < props.enhance.final) {
        ...
        if (chance <= upgradeChance) {
            props.enhance.now!++;
            props.allChances[options[props.enhance.now as AllowedRefinesFinal].to as 'nine' | 'ten' | 'eleven' | 'twelve' | 'thirteen'].push({ case: caso, fluorite: props.resources.fluorite, crystal: props.resources.crystal, blessedScroll: props.resources.blessedScroll });
            ...
            props.allChances[options[props.enhance.now! + 1 as AllowedRefinesFinal].to as 'nine' | 'ten' | 'eleven' | 'twelve' | 'thirteen'].push({ case: caso, fluorite: props.resources.fluorite, crystal: props.resources.crystal, blessedScroll: props.resources.blessedScroll });
            props.resources.blessedScroll = 0;
            props.resources.fluorite = 0;
            props.resources.crystal = 0;
            props.enhance.now!--;
